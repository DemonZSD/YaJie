	...
	for(var i=0; i<menu.length; i++){   
		...
		try{
			if(item.isHeader == "1"){
				...
				if(item.childMenus == ""){
					str = "<li><a href='"+item.url+"'><i class='fa "+item.icon+"'></i><span>"+item.name+"</span></a></li>";
					$(parent).append(str);
				}else{
					str = "<li><a href='"+item.url+"'><i class='fa "+item.icon+"'></i><span>"+item.name+"</span><i class='icon-font icon-right'>&#xe60b;</i></a>";
...
$(function(){
	...
	var menu = [{"id":"1","name":"主菜单","parentId":"0","url":"","icon":"","order":"1","isHeader":"1","childMenus":[
					{"id":"3","name":"商品管理","parentId":"1","url":"","icon":"fa-gears","order":"1","isHeader":"0","childMenus":[
